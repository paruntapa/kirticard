...
const CategoryCardCarousel = ({ cards, title, currentIndex, onNext, onPrev }: {
    cards: Card[];
    title: string;
    currentIndex: number;
    onNext: () => void;
    ...
    if (visibleCards.length === 0) {
      return (
        <div className="text-center py-12">
          <p className="text-gray-500 text-lg">No cards found for this category.</p>
        </div>
      );
    }
              ...
              <div key={card.id} className="flex-none w-full md:w-1/2 lg:w-1/3">
                <Card className="group overflow-hidden border-0 shadow-lg hover:shadow-2xl transition-all duration-300 h-full">
                    ...
                    <div className="relative">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-sm font-semibold">{card.starRating}</span>
                        </div>
                      </div>
                      <div className="w-full h-24 hover:scale-105 transition-all duration-300 rounded-lg mb-4 flex items-center justify-center">
                        <img 
                          src={card.image} 
                          alt={card.cardName}
                  ...
                  <CardContent className="p-6 space-y-4 flex-1">
                    ...
                    <Button className="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:opacity-90 text-white mt-auto">
                      Apply Now
                    </Button>
                  </CardContent>
                </Card>
